...
export default function ScanPage() {
  ...
  const togglePassport = index => {
    setImages(prev => prev.map((img, i) => i === index ? { ...img, isPassport: !img.isPassport } : img));
  };

  const deletePhoto = index => setImages(prev => prev.filter((_, i) => i !== index));
    ...
    try {
      for (const img of images) {
        ...
        if (blob.size > MAX_FILE_SIZE) {
          ...
          continue; // или break, если нужно прервать полностью
        }
  ...
  return (
    <div className="scan-page">
      <header className="header">
        ...
        <img src='/logout_icon.png' className="logout-btn" onClick={handleLogout} />
      </header>
      ...
      <div className="viewfinder-container">
        {scanning ? (
          ...
          <button className="action-btn" onClick={startScanning}>Начать сканирование QR</button>
        )}
      </div>
      ...
      <div className="controls">
        ...
        {clientId && (
          <div className="btn-group">
            <button className="action-btn" onClick={takePhoto}>Сделать фото</button>
            <button className="action-btn" onClick={resetScan}>Новый QR</button>
          </div>
        )}
      </div>
        ...
        {images.map((img, i) => (
          <div key={i} className="gallery-item">
            ...
            <label className="checkbox-label">
              <input type="checkbox" checked={img.isPassport} onChange={() => togglePassport(i)} />
              Паспорт
            </label>
            <button onClick={() => deletePhoto(i)} className="delete-btn">×</button>
          </div>
        ))}
      </div>
